...
class Vector():
    ...
    def proj_vek(cls, v1, v2):
        ...
        skalar = (v1.x * v2.x + v1.y * v2.y + v1.z * v2.z) / (math.sqrt((v2.x)**2 + (v2.y)**2 + (v2.z)**2))
...
'''
Opgave 14
     Tilføj en funktion til at beregne vinklen mellem to linjer 
     (Måske har du allerede en der beregner vinklen mellem to vektorer)
     Tilføj en docstring. Kan din funktion beregne vinkler hvis den får to parameterfremstillinger? 
     Kan den hvis den får to punkter på de to linjer?
