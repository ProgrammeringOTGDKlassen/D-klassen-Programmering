...
class Vector:
    ...
    def normalize(self):  # Function is fucked TODO
        """
...
class Line:
    ...
    def __str__(self):
        return "(x, y, z) = ({}, {}, {}) + t*({}, {}, {})".format(
            self.p0.x, self.p0.y, self.p0.z, self.d.x, self.d.y, self.d.z
        )
...
def distancePointPlane(p: Point, a: Plane) -> float:
    ...
    # normal vectors
    return math.fabs(
        (a.normal() * Vector.connect(p.x, p.y, p.z, a.p0.x, a.p0.y, a.p0.z))
    )
...
class Matrix:
    ...
    def gauss(self):
        elementaries = (
            list()
        )  # List of all elementary matrices that are multiplied with the identity matrix to get to Reduced Row-Echelon Form
        print(self.vectors)
        for i in range(0, self.m):
            ...
            for k in range(i, self.m + 1):
                ...
                elementaries.append(Matrix.identity_matrix(self.m).row_swap(maxRow, i))
            ...
            for k in range(i + 1, self.m):
                ...
                for j in range(i, self.m + 1):
                    if i == j:
                        ...
                        elementaries.append(
                            Matrix.identity_matrix(self.m).row_scale(k, c)
                        )
        ...
        for i in range(self.m - 1, -1, -1):
            ...
            for k in range(i - 1, -1, -1):
                ...
                elementaries.append(
                    Matrix.identity_matrix(self.m).scaled_row_addition(i, k, -x[i])
                )

        inverse = Matrix.identity_matrix(self.n)
        for element in elementaries:
            inverse = inverse * element
        result = Matrix.identity_matrix(self.m).append_column(Vector(x))
        return result, inverse, elementaries
    ...
    def get_rotation_matrix(self, angleX, angleY, angleZ):
        v1 = Vector(
            ...
            math.sin(angleZ) * math.cos(angleY),
            -1 * math.sin(angleY),
        )
        v2 = Vector(
        ...
        v3 = Vector(
            math.cos(angleZ) * math.sin(angleY) * math.cos(angleX)
            + math.sin(angleZ) * math.sin(angleX),
            math.sin(angleZ) * math.sin(angleY) * math.cos(angleX)
            - math.cos(angleZ) * math.sin(angleX),
