...
class Robot_programmer():
    ...
    def move_relative_z(self, z):
        if self.connected:
            ...
            self.s.send(b' movel(var_1)\n')
            self.s.send(b'end\n')
    def move_relative_xyz(self, x, y, z):
        if self.connected:
            self.s.send(b'def myProg():\n')
            self.s.send(b' var_1=get_actual_tcp_pose()\n')
            st= '  var_1[0] = var_1[0] + {}\n'.format(x)
            ...
            self.s.send(b' movel(var_1)\n')
            self.s.send(b'end\n')
    def move_relative_xyza(self, x, y, z, rz):
        if self.connected:
            self.s.send(b'def myProg():\n')
            self.s.send(b' var_1=get_actual_tcp_pose()\n')
            st= '  var_1[0] = var_1[0] + {}\n'.format(x)
