        ...
        for x in range(0, len(self.grid)):
            for y in range(0, len(self.grid)):
                if self.grid[x][y].color == 0:
                    if y < len(self.grid[x]) - 1 and not all(
                        self.grid[x][yy].color == 0
                        for yy in range(y, len(self.grid[x]))
                    ):
                        # Flyt kolonnen ned
                        while self.grid[x][y].color == 0:
                            ...
                            if self.user_press:
                                self.anim[x][y:] = [
                                    50 for i in range(y, len(self.anim[x]))
                                ]
    ...
    def detect_matches(self):
        for x in range(1, len(self.grid) - 1):
            for y in range(0, len(self.grid)):
                ...
                if (
                    self.grid[x][y].color == self.grid[x - 1][y].color
                    and self.grid[x][y].color == self.grid[x + 1][y].color
                ):
        ...
        for y in range(1, len(self.grid) - 1):
            for x in range(0, len(self.grid)):
                ...
                if (
                    self.grid[x][y].color == self.grid[x][y - 1].color
                    and self.grid[x][y].color == self.grid[x][y + 1].color
                ):
                    c = self.grid[x][y].color
                    match_list = [
                        self.grid[x][y - 1],
