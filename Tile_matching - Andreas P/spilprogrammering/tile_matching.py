...
def draw_game():
    ...
    if current_tile is not None:
        ...
        pygame.draw.rect(
            ...
            c,
            pygame.Rect(
                tile_offset[0] + current_tile[0] * tile_size[0] - 3,
                tile_offset[1] - (current_tile[1] + 1) * tile_size[1] - 3,
                tile_size[0],
                tile_size[1],
            ),
        )
    ...
    if (
        restart_pos[0] < pos[0] < restart_pos[0] + restart_dim[0]
        and restart_pos[1] < pos[1] < restart_pos[1] + restart_dim[1]
    ):
        pygame.draw.rect(
            screen,
            (255, 220, 115),
    ...
    if (
        reset_score_pos[0] < pos[0] < reset_score_pos[0] + reset_score_dim[0]
        and reset_score_pos[1] < pos[1] < reset_score_pos[1] + reset_score_dim[1]
    ):
        pygame.draw.rect(
            screen,
            (255, 220, 115),
...
while not done:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if (
                reset_score_pos[0] < pos[0] < reset_score_pos[0] + reset_score_dim[0]
                and reset_score_pos[1]
                < pos[1]
                < reset_score_pos[1] + reset_score_dim[1]
            ):
                game.points = 0
